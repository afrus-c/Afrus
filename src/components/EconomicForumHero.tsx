...
import fintechImg from '../assets/images/fintech_banking_network_1785939957278.jpg';
import tradeFinanceImg from '../assets/images/commercial_trade_finance_1785939987782.jpg';
import industrialMachineryImg from '../assets/images/commodity_industrial_machinery_1785939080467.jpg';
import { useTranslation } from 'react-i18next';
import { getPageHeroImage } from '../utils/pageMedia';

interface EconomicForumHeroProps {
  onOpenInquiry: (subject?: string) => void;
}

const SLIDES = [
  {
    id: 'bilateral-banking',
    image: getPageHeroImage('forum', 0, fintechImg),
    key: 'summits'
  },
  {
    id: 'trade-finance',
    image: getPageHeroImage('forum', 1, tradeFinanceImg),
    key: 'finance'
  },
  {
    id: 'industrial-roundtables',
    image: getPageHeroImage('forum', 2, industrialMachineryImg),
    key: 'industry'
  },
  {
    id: 'global-economic-plenary',
    image: getPageHeroImage('forum', 3, 'https://images.unsplash.com/photo-1540575467063-178a50c2df87?auto=format&fit=crop&w=1920&q=80'),
    key: 'plenary'
  }
];

const NAV_CHIPS = [
  { id: 'summit-schedule', key: 'agenda', icon: Users },
  { id: 'roundtables', key: 'roundtables', icon: Landmark },
  { id: 'delegation-access', key: 'delegation', icon: Award },
  { id: 'matchmaking', key: 'matchmaking', icon: TrendingUp }
];

export const EconomicForumHero: React.FC<EconomicForumHeroProps> = ({ onOpenInquiry }) => {
  const { t } = useTranslation();
  const [currentSlide, setCurrentSlide] = useState(0);
  const [isPlaying, setIsPlaying] = useState(true);
  const [activeNav, setActiveNav] = useState('summit-schedule');
  useEffect(() => {
    if (!isPlaying) return;
  ...
  return (
    <section className="relative min-h-[88vh] flex flex-col justify-between bg-slate-950 border-b border-slate-800/80 overflow-hidden pt-20">
      <div className="absolute inset-0 z-0">
        <AnimatePresence mode="wait">
          ...
          >
            <img
              ...
              alt={t(`forumPage.hero.slides.${SLIDES[currentSlide].key}.caption`)}
        ...
        >
          <path d="M 120 120 L 880 120" stroke="rgba(0, 57, 166, 0.25)" strokeWidth="1" strokeDasharray="6 6" />
          <path d="M 120 320 L 880 320" stroke="rgba(0, 57, 166, 0.2)" strokeWidth="1" strokeDasharray="6 6" />
          <path d="M 120 520 L 880 520" stroke="rgba(0, 57, 166, 0.15)" strokeWidth="1" strokeDasharray="6 6" />
          ...
          <circle cx="280" cy="380" r="5" fill="#38bdf8" />
          <circle cx="280" cy="380" r="14" fill="none" stroke="#38bdf8" strokeWidth="1" opacity="0.6">
            <animate attributeName="r" values="6;20;6" dur="3.5s" repeatCount="indefinite" />
          </circle>

          <circle cx="720" cy="380" r="5" fill="#0039a6" />
          <circle cx="720" cy="380" r="14" fill="none" stroke="#0039a6" strokeWidth="1" opacity="0.6">
            <animate attributeName="r" values="6;20;6" dur="4.5s" repeatCount="indefinite" />
          </circle>
        </svg>
      </div>
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 flex-1 flex flex-col justify-center items-center text-center">
        ...
        >
          ...
          <span>{t('forumPage.hero.badge')}</span>
        </motion.div>
        <motion.h1
        ...
        >
          {t('forumPage.hero.titlePrefix')} <br />
          <span className="bg-gradient-to-r from-amber-300 via-amber-400 to-amber-500 bg-clip-text text-transparent">
            {t('forumPage.hero.titleAccent')}
          </span>
        </motion.h1>
        <motion.p
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          <button
            onClick={() => onOpenInquiry(t('forumPage.subjects.access'))}
          ...
          >
            <span>{t('forumPage.hero.primaryCta')}</span>
            <ArrowRight className="w-4 h-4 group-hover:translate-x-1 transition-transform" />
          </button>

          <button
            onClick={() => scrollToSection('summit-schedule')}
          ...
          >
            ...
            <span>{t('forumPage.hero.secondaryCta')}</span>
          </button>
        </motion.div>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          <div className="flex items-center gap-1.5">
            {SLIDES.map((slide, idx) => (
              <button
                ...
                title={t(`forumPage.hero.slides.${slide.key}.tag`)}
                aria-label={t('forumPage.hero.slideLabel', { number: idx + 1 })}
              />
            ))}
          </div>
          ...
          <span className="text-[11px] font-semibold text-slate-300 tracking-wide">
            {t(`forumPage.hero.slides.${SLIDES[currentSlide].key}.tag`)}
          </span>
          ...
          <button
            ...
            aria-label={isPlaying ? t('forumPage.hero.pause') : t('forumPage.hero.play')}
          >
