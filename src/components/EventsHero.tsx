...
import cultureFestivalImg from '../assets/images/culture_festival_exchange_1785925175774.jpg';
import bilateralBridgeImg from '../assets/images/hero_russia_africa_bridge_1785919860735.jpg';
import artCraftsImg from '../assets/images/african_art_crafts_1785919917844.jpg';
...
const SLIDES = [
  {
    id: 'diplomatic-summits',
    image: getPageHeroImage('events', 0, bilateralBridgeImg),
    key: 'summits'
  },
  {
    id: 'cultural-exchanges',
    image: getPageHeroImage('events', 1, cultureFestivalImg),
    key: 'festivals'
  },
  {
    id: 'artisan-expos',
    image: getPageHeroImage('events', 2, artCraftsImg),
    key: 'expos'
  },
  {
    id: 'convention-stage',
    image: getPageHeroImage('events', 3, 'https://images.unsplash.com/photo-1511578314322-379afb476865?auto=format&fit=crop&w=1920&q=80'),
    key: 'plenary'
  }
];

const NAV_CHIPS = [
  { id: 'upcoming-events', key: 'upcoming', icon: Calendar },
  { id: 'business-forums', key: 'forums', icon: Landmark },
  { id: 'cultural-festivals', key: 'festivals', icon: Users },
  { id: 'exhibitions-fairs', key: 'exhibitions', icon: Award }
];

export const EventsHero: React.FC<EventsHeroProps> = ({ onOpenInquiry }) => {
  const { t } = useTranslation();
  const [currentSlide, setCurrentSlide] = useState(0);
  const [isPlaying, setIsPlaying] = useState(true);
  const [activeNav, setActiveNav] = useState('upcoming-events');
  useEffect(() => {
    if (!isPlaying) return;
  ...
  return (
    <section className="relative min-h-[88vh] flex flex-col justify-between bg-slate-950 border-b border-slate-800/80 overflow-hidden pt-20">
      <div className="absolute inset-0 z-0">
        <AnimatePresence mode="wait">
          ...
          >
            <img
              ...
              alt={t(`eventsPage.hero.slides.${SLIDES[currentSlide].key}.caption`)}
        ...
        >
          <path d="M 100 200 L 900 200" stroke="rgba(0, 57, 166, 0.25)" strokeWidth="1" strokeDasharray="6 6" />
          <path d="M 100 400 L 900 400" stroke="rgba(0, 57, 166, 0.2)" strokeWidth="1" strokeDasharray="6 6" />
          <circle cx="500" cy="300" r="8" fill="#0039a6" />
          <circle cx="500" cy="300" r="22" fill="none" stroke="#0039a6" strokeWidth="1" opacity="0.6">
            <animate attributeName="r" values="10;32;10" dur="4s" repeatCount="indefinite" />
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 sm:py-16 flex-1 flex flex-col justify-center items-center text-center">
        ...
        >
          ...
          <span>{t('eventsPage.hero.badge')}</span>
        </motion.div>
        <motion.h1
        ...
        >
          {t('eventsPage.hero.titlePrefix')} <br />
          <span className="bg-gradient-to-r from-amber-300 via-amber-400 to-amber-500 bg-clip-text text-transparent">
            {t('eventsPage.hero.titleAccent')}
          </span>
        </motion.h1>
        <motion.p
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          <button
            onClick={() => onOpenInquiry(t('eventsPage.hero.subject'))}
          ...
          >
            ...
            <span>{t('eventsPage.hero.primaryCta')}</span>
          ...
          >
            <Calendar className="w-4 h-4 text-amber-400" />
            <span>{t('eventsPage.hero.secondaryCta')}</span>
          </button>
        </motion.div>
        <motion.div
          initial={{ opacity: 0, y: 20 }}
        ...
        >
          <div className="flex items-center gap-1.5">
            {SLIDES.map((slide, idx) => (
              <button
                ...
                title={t(`eventsPage.hero.slides.${slide.key}.tag`)}
                aria-label={t('eventsPage.hero.slideLabel', { number: idx + 1 })}
              />
            ))}
          </div>
          ...
          <span className="text-[11px] font-semibold text-slate-300 tracking-wide">
            {t(`eventsPage.hero.slides.${SLIDES[currentSlide].key}.tag`)}
          </span>
          ...
          <button
            ...
            aria-label={isPlaying ? t('eventsPage.hero.pause') : t('eventsPage.hero.play')}
          >
